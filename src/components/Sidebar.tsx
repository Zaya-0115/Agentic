...
export default function Sidebar({ active, onNavigate, cartCount = 0 }: Props) {
  return (
    <nav className="w-16 shrink-0 h-screen bg-[#1a1a2e] flex flex-col items-center py-5 gap-1">
        ...
        <button key={item.id} onClick={() => onNavigate(item.id)} title={item.label}
          className={`relative rounded-xl flex items-center justify-center transition-all ${
            item.id === "cart" && cartCount > 0 && active === "cart"
              ? "w-12 h-12 bg-primary text-white shadow-lg shadow-primary/30"
      ...
      {/* Chat */}
      <button onClick={() => onNavigate("chat")} title="Чат"
        className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all mb-2 ${
          active === "chat" ? "bg-white/15 text-white" : "text-white/40 hover:text-white/70 hover:bg-white/10"
        }`}>
      ...
      {/* Profile */}
      <button className="w-9 h-9 rounded-full bg-gradient-to-br from-gray-300 to-gray-400 flex items-center justify-center mb-2 hover:ring-2 hover:ring-primary/30 transition-all overflow-hidden">
